 ...
 * from JavaScript.
 */
public class X5PluginManager {
    private static String TAG = "PluginManager";
    private static final int SLOW_EXEC_WARNING_THRESHOLD = Debug.isDebuggerConnected() ? 60 : 16;

    // List of service entries
    private final LinkedHashMap<String, X5CordovaPlugin> pluginMap = new LinkedHashMap<String, X5CordovaPlugin>();
    private final LinkedHashMap<String, X5PluginEntry> entryMap = new LinkedHashMap<String, X5PluginEntry>();

    private final X5CordovaInterface ctx;
    private final X5CordovaWebView app;
    private boolean isInitialized;

    private X5CordovaPlugin permissionRequester;

    public X5PluginManager(X5CordovaWebView cordovaWebView, X5CordovaInterface cordova, Collection<X5PluginEntry> pluginEntries) {
        this.ctx = cordova;
        this.app = cordovaWebView;
        setPluginEntries(pluginEntries);
    }

    public Collection<X5PluginEntry> getPluginEntries() {
        return entryMap.values();
    }

    public void setPluginEntries(Collection<X5PluginEntry> pluginEntries) {
        if (isInitialized) {
            this.onPause(false);
            this.onDestroy();
            pluginMap.clear();
        ...
        for (X5PluginEntry entry : pluginEntries) {
            addService(entry);
        }
        if (isInitialized) {
            startupPlugins();
    ...
    private void startupPlugins() {
        for (X5PluginEntry entry : entryMap.values()) {
    ...
    public void exec(final String service, final String action, final String callbackId, final String rawArgs) {
        X5CordovaPlugin plugin = getPlugin(service);
        if (plugin == null) {
        ...
        X5CallbackContext callbackContext = new X5CallbackContext(callbackId, app);
        try {
     ...
     */
    public X5CordovaPlugin getPlugin(String service) {
        X5CordovaPlugin ret = pluginMap.get(service);
        if (ret == null) {
            X5PluginEntry pe = entryMap.get(service);
            if (pe == null) {
                return null;
            }
            if (pe.plugin != null) {
    ...
    public void addService(String service, String className) {
        X5PluginEntry entry = new X5PluginEntry(service, className, false);
     ...
     */
    public void addService(X5PluginEntry entry) {
    ...
    public void onPause(boolean multitasking) {
        for (X5CordovaPlugin plugin : this.pluginMap.values()) {
            if (plugin != null) {
    ...
    public boolean shouldAllowRequest(String url) {
        for (X5PluginEntry entry : this.entryMap.values()) {
            X5CordovaPlugin plugin = pluginMap.get(entry.service);
            if (plugin != null) {
     ...
     */
    private X5CordovaPlugin instantiatePlugin(String className) {
        X5CordovaPlugin ret = null;
        try {
            Class<?> c = null;
            if ((className != null) && !("".equals(className))) {
                c = Class.forName(className);
            }
            if (c != null & X5CordovaPlugin.class.isAssignableFrom(c)) {
                ret = (X5CordovaPlugin) c.newInstance();
            }
        } catch (Exception e) {
            e.printStackTrace();
